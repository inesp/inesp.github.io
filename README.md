...
- Configure Bundler to install gems locally (avoids permission issues): `bundle config set --local path 'vendor/bundle'`
- Install all the gems in the `Gemfile`: `bundle install`
- Run the server: `make up` (or `bundle exec jekyll serve --future`)
...
Every so often run `make upgrade` (or `bundle update`) and then commit the new generated `Gemfile.lock`.
